...
for (var i = 0; i < hands.length; i++) {
  ...
  if (hand.includes('J') && !hand.startsWith('JJJJJ')) {
    var best = {};
    for (const card in cards) if (card != 'J' && (best.value == null || cards[card] > best.value || (cards[card] == best.value && tiebreaker(card) > tiebreaker(best.key)))) best = { key: card, value: cards[card] };
    console.log(best)
